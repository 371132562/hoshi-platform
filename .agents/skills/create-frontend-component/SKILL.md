...
6. 常规列表页（顶部工具栏含搜索、新建等操作）中的 Ant Design `Table` 默认补齐 `scroll={{ x: 'max-content', y: 'calc(100vh - 360px)' }}`；日志/特殊面板按实际布局单独计算，不强行套用该值。
7. 常规列表页若只处理分页，统一使用 `pagination.onChange`；只有确实要联动排序、筛选时才改用顶层 `Table onChange`。
8. 修改页面入口时同步检查 `router/` 的菜单、面包屑与权限字段。
9. 类型契约细则转 `type-contract-guidelines`。
10. 后台路由相关字段同步检查 `adminOnly`、`menuParent`、`hideInBreadcrumb`。
11. 复杂页面优先拆页面内子组件或公共组件，不要把大段 JSX 分支堆进单文件。
12. 加载态优先骨架屏或现有 Loading 组件，文案统一中文。
