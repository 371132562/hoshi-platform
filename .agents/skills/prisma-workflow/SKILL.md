...
3. 可按最小必要范围执行 Prisma 命令，例如 `generate`、`migrate dev`、`migrate deploy`、`db push`、`db seed`。
4. 如涉及迁移，明确迁移策略与回滚方式；不要因为“顺手验证”扩大到无关的 install / dev / build 命令。
...
- Prisma 命令属于受控例外，只能在数据库相关任务中执行，并且优先选择最小、最直接的那一条命令。
